...
It implements Operations Manager RESTful API RAML specification, which can be
found here: http://ftp.opengear.com/download/api/operations_manager/
"""
...
class OperationsManagerClient:
    """
    The basic API client, with methods for GET, POST, PUT, and DELETE.
    """

    def __init__(self, url="", username="", password=""):
        self.url = url if url else os.environ.get('OGOM_API_URL')
        self.username = username if username else os.environ.get('OGOM_API_USER')
        self.password = password if password else os.environ.get('OGOM_API_PASS')

        if not (self.url and self.username and self.password):
            raise RuntimeError("""
    Missing some of the required environment variables. Please refer
    to the documentation at https://github.com/opengear/ogomclient""")

        requests.packages.urllib3.disable_warnings()
        self.api_url = self.url + '/api/v2.2'
        self.token = None
        ...
        try:
            ...
            r = self.s.get('http://ftp.opengear.com/download/api/operations_manager/og-rest-api-specification-v2-2-ngcs.raml')
            self.raml = yaml.load(re.sub('\t','  ',re.sub('\\\/','/',re.sub(':\"',': \"',r.text))),
                Loader=yaml.FullLoader)

        if not isinstance(self.raml, dict):
            raise RuntimeError("""
    Operations Manager RESTful API specification couldn't be loaded, locally nor
    at http://ftp.opengear.com/download/api/operations_manager""")
    ...
    def _get_client(self, node, path):
        ...
        SynClient = namedtuple('OgOMClient', ' '.join(kwargs.keys()))
